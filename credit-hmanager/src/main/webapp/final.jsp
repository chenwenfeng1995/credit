...
<script type="text/javascript">
	function selectLike(){
		...
		location.href="/final/Querycredit?realname="+realname+"&mobile="+mobile
	}
</script>
	...
	<div class="header">
		...
		<div class="header-b">
			<div class="w1200 auto">
				<div class="nav">
					<ul class="auto">
						<li class="selected"><a href=""><span class="icn1"></span><i>审核信息</i></a></li>
		...
		<div class="c-left">
			...
			<div class="l-menu">
				<ul class="nav_li">
					...
					<li class="selected"><a href="final/Querycredit">最终审核</a></li>
				</ul>
			</div>
		</div>
		<!-- 左侧end -->
			...
			<div class="search-member">
				<dl>
					...
					<form action="" method="post">
						<label for="">
							<p class="mt5">申请人：</p> <input type="text"  name="realname" id="realname"
							class="input02 w200 h30 cus1">
						</label> <label for="">
							<p class="mt5">手机号：</p> <input type="text" name="mobile" id="mobile"
							class="input02 w200 h30 cus1">
						</label> <span class="search-b"> <input type="button" value="搜 索" onclick="selectLike()"/>
							<i></i>
						</span>
					</form>
					</dd>
...
<span><font color=red id="info"  size="3"  >*说明:性别:男(F),女(M);Y(是),N(否)</font></span>
			<div class="lib-tab5 xg-fb">
				<table width="100%">
					...
					<c:forEach items="${pageInfo.list}" var="example">
						<tr>
							<td>${example.realname}</td>
							<td>${example.borrowmoney}</td>
							<td><fmt:formatDate value="${example.borrowtime}" pattern="yyyy-MM-dd"/></td>
							<td><fmt:formatDate value="${example.paymenttime}" pattern="yyyy-MM-dd"/></td>
							<td>${example.phone}</td>
							<td><fmt:formatDate value="${example.createtime}" pattern="yyyy-MM-dd"/></td>
							<td class="c-z"><a href="/final/finaldetails?userid=${example.userid}">审核</a></td>
						</tr>
					</c:forEach>
       ...
       <a href="/final/Querycredit?pageNo=1">第一页</a>
        <c:if test="${pageInfo.hasPreviousPage }">
			<a href="/final/Querycredit?pageNo=${pageInfo.pageNum-1}">上一页</a>
		</c:if>
      
        <c:if test="${pageInfo.hasNextPage }">
			<a href="/final/Querycredit?pageNo=${pageInfo.pageNum+1}">下一页</a>
		</c:if>
        
        <a href="/final/Querycredit?pageNo=${pageInfo.pages}">最后页</a>
